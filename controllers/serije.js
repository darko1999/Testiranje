...
const vratiSerijuPoNazivu = async (req, res, next) => {
  const serija = sveSerije.find(
    (serija) => serija.id === parseInt(req.params.id)
  );
  if (!serija) {
...
const vratiOpisSerije = async (req, res, next) => {
  ...
  if (!serija) {
    ...
    const plot = { Ime: serija.title, Opis: serija.plot };
...
const vratiEpizodeSerije = async (req, res, next) => {
  ...
  if (!serija) {
    ...
    const epizode = { Ime: serija.title, Epizode: serija.episodes };
